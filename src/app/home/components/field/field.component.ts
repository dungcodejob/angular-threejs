...
export class FieldComponent implements OnInit {
  ...
  private _separation(currentBoid: Boid): Vector2 {
    ...
    for (const boid of this.boids) {
      ...
      const distance = currentBoid.position.distanceTo(boid.position);
  ...
  private _predator(currentBoid: Boid): Vector2 {
    const predatorVelocity = new Vector2(0, 0);

    for (const predator of this.predators) {
      const distance = currentBoid.position.distanceTo(predator.position);
    ...
    const distanceMouse = currentBoid.position.distanceTo(new Vector2(x, y));
  ...
  private _limitSpeed(boid: Boid) {
    ...
    if (speed > this.maxSpeed) {
      boid.velocity = boid.velocity.divideScalar(speed).multiplyScalar(this.maxSpeed);
    }

    if (speed < this.minSpeed) {
      boid.velocity = boid.velocity.divideScalar(speed).multiplyScalar(this.minSpeed);
    }
  }
